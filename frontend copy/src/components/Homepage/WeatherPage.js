...
import { Grid, Image, Button, Search, Input, Label, Popup } from 'semantic-ui-react'
...
export default function WeatherPage() {
	...
	let API_address = "http://api.openweathermap.org/data/2.5/weather?q="
	//http://api.openweathermap.org/data/2.5/weather?q=Houston&appid=03b80556af981e9f9a6a57906ecf2438
	useEffect(() => {
		fetch(`${API_address}${localStorage.city}&units=imperial&appid=${API_key}`)
			.then(res => res.json())
			.then(weather => {
    ...
    console.log(localStorage.city, weather)
    if ( weather.temperature == "" ) {
        return <h1>Loading...</h1>
    }
    return (
        <div>
            <h1>Today's forecast for {localStorage.city} </h1>
            <Label size="big">Current temperature in {localStorage.city}</Label>
